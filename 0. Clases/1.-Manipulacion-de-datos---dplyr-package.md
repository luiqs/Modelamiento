...
-   Si queremos usar un operador contrario al %in%, podemos crearlo como
    función de la siguiente manera (hay que crearlo y correrlo antes de
    usarlo, su uso sigue la misma logica que el operador original).
...
# D. Recodificación condicional en R (variantes de la función mutate())

Aveces queremos crear una nueva columna “categorica” en nuestra base de
datos en relación con algun valor cuantitativo que tengamos en nuestra
base de datos original. Ejemplo:

``` r
library(knitr)
Base_original <- tibble::tibble(Nombre = c("Sara", "Estafani","Steve", "Jason", "Mateo", "Rosa"),
                                Número_caramelos = c(7, 5, 4, 6, 3, 5))
kable(Base_original)
```

| Nombre   | Número\_caramelos |
|:---------|------------------:|
| Sara     |                 7 |
| Estafani |                 5 |
| Steve    |                 4 |
...
Supongamos que de esta base de datos, queremos adicionar una tercera
columna que clasifique y número de caramelos consumidos, pero evaluado
con una variable categorica. Podemos realizar esto de la siguiente
manera, con codigo de R basico:

``` r
Base_original$Caramelos_consumidos[Base_original$Número_caramelos < 5] <- "Debajo del promedio"
```

    ## Warning: Unknown or uninitialised column: `Caramelos_consumidos`.

``` r
Base_original$Caramelos_consumidos[Base_original$Número_caramelos > 5] <- "Encima del promedio"
Base_original$Caramelos_consumidos[Base_original$Número_caramelos == 5] <- "Promedio"
```

Si visualizamos nuevamente la base de datos que creamos, veran una nueva
columna creada en relación a las condiciones que brindamos.

``` r
kable(Base_original)
```

| Nombre   | Número\_caramelos | Caramelos\_consumidos |
|:---------|------------------:|:----------------------|
| Sara     |                 7 | Encima del promedio   |
| Estafani |                 5 | Promedio              |
| Steve    |                 4 | Debajo del promedio   |
| Jason    |                 6 | Encima del promedio   |
| Mateo    |                 3 | Debajo del promedio   |
| Rosa     |                 5 | Promedio              |

Este mismo ejercicio podemos realizarlo con la función “mutate()” del
paquete dplyr:

``` r
library(dplyr)
Base_original_EjemploMutate <- tibble::tibble(Nombre = c("Sara", "Estafani","Steve", "Jason", "Mateo", "Rosa"),
                                Número_caramelos = c(7, 5, 4, 6, 3, 5))

Recodificación <- Base_original_EjemploMutate %>% 
        dplyr::mutate(Caramelos_consumidos = case_when(Número_caramelos < 5 ~ "Debajo del promedio",
                                                       Número_caramelos > 5 ~ "Encima del promedio",
                                                       Número_caramelos == 5 ~ "Promedio"))
```

``` r
kable(Recodificación)
```
